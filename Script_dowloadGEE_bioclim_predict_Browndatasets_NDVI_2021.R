...
####The predictors and responses should be the same resolution. Can use GEE dowload to fix diversity ones or viceversa it is donde here with test_ras based on diversity
...
###NDVI data
#source_python("~/Dropbox/**Tesis_PHD/Near_real_time_model/Python_GEE_scripts/GetNDVI.py")
source_python("~/Andrea/Near_real_time_model/GetNDVI.py")

#######################################################################################
###WARNING: after sourcing python scripts must stop until all layers are generated! 
###If not then not all layers wqill be dowloaded prior to analyses
#######################################################################################
...
###one per year for NDVI
NDVI_layers<-stack(All_layers)[[which(grepl("NDVI",All_names))]]
...
###separate NDVI into test/train (check if it does correctly before running)

NDVI_layers_train<-NDVI_layers[[which(grepl(paste(trainV,collapse="|"),names(NDVI_layers)))]]
NDVI_layers_test<-NDVI_layers[[which(grepl(paste(testV,collapse="|"),names(NDVI_layers)))]]
...
##Add NDVI median for the year to the prediction
bioclimatic<-stack(bioclimatic,NDVI_layers_train)
library(usdm)
viRes<-vifstep(bioclimatic)
bioclimatic<-subset(bioclimatic,as.character(viRes@results$Variables))

##Here add stack of stacks with all test layers in one year (one per)
##All_test


##multiply by 0.02 and then substract -273.15 to get to Celsius

# Predicting diversity variables using climatic layers
# Thiago Sanna F. Silva -  tsfsilva@rc.unesp.br
##Modelol lineal + GAMM still not implemented just RF ,NN and SVM
# Loading libraries
library(raster)
library(caret)
library(tools)
...
for (i in 1:length(div.names)){
  ...
  input.df.na<-input.df.na%>%drop_na(7:15) ##this assumes 9 predictors else must change numbers
...
#  plot.df <- gather(input.df.na[,c(i,2:length(input.df.na))], key = predictor, value = value, -1)
 ...
 # fit.control <- trainControl(method='repeatedcv',number=10,repeats=3,savePredictions="final") ##here method is overruled by indices so any method can be written or nothing
  ##Now using blocks
...
#def more standard 
 ...
 # m.bigRF$results
  ...
  write.csv(stack.glm$error,paste("/Users/andreapaz/Dropbox/**Tesis_PHD/Near_real_time_model/results_part/","result_Ensemble_",names(input.df.na[i]),".csv",sep=""))
  ...
  writeRaster(observed.stack,paste("/Users/andreapaz/Dropbox/**Tesis_PHD/Near_real_time_model/results_part/","result_stack_observed",names(input.df.na[i]),".asc",sep=""),format="ascii",overwrite=T)
  writeRaster(predicted.stack,paste("/Users/andreapaz/Dropbox/**Tesis_PHD/Near_real_time_model/results_part/","result_stack_predicted_original",names(input.df.na[i]),".asc",sep=""),format="ascii")
  jpeg(paste("/Users/andreapaz/Dropbox/**Tesis_PHD/Near_real_time_model/results_part/","result_stack_original",names(input.df.na[i]),".jpg",sep="_"))
  ...
  for (j in testV){
    if( length(which(grepl(paste("test",j,sep="_"),test.files)))>0){
      ...
      writeRaster(observed.stack,paste("/Users/andreapaz/Dropbox/**Tesis_PHD/Near_real_time_model/results_part/","result_stack_observed",names(input.df.na[i]),".asc",sep=""),format="ascii",overwrite=T)
      writeRaster(predicted.stack,paste("/Users/andreapaz/Dropbox/**Tesis_PHD/Near_real_time_model/results_part/",paste("result_stack_predicted",j,sep="_"),names(input.df.na[i]),".asc",sep=""),format="ascii")
      ...
      jpeg(paste("/Users/andreapaz/Dropbox/**Tesis_PHD/Near_real_time_model/results_part/","result_stack",names(input.df.na[i]),j,".jpg",sep="_"))
...
##create one figure with all projections
 ...
 # jpeg(paste("~/Dropbox/**Tesis_PHD/Near_real_time_model/results/","result_stack",names(input.df.na[i]),"all",".jpg",sep="_"))
  jpeg(paste("/Users/andreapaz/Dropbox/**Tesis_PHD/Near_real_time_model/results_part/","result_stack",names(input.df.na[i]),"all",".jpg",sep="_"))
...
#  save.image(paste("~/Dropbox/**Tesis_PHD/Near_real_time_model/",names(input.df.na[i]),"_workspace.RData",sep=""))
  write.csv(stack.varimp,paste("/Users/andreapaz/Dropbox/**Tesis_PHD/Near_real_time_model/results_part/",names(input.df.na[i]),"stack_variable_contribution",".csv",sep=""))
  
  save.image(paste("/Users/andreapaz/Dropbox/**Tesis_PHD/Near_real_time_model/results_part/",names(input.df.na[i]),"_workspace.RData",sep=""))
...
############THIS is completly independent!###################

###For making graphs but this hast to be done independently for each method
####Load result files and create a single dataframe with them
results<-list.files(pattern=".csv")[33:68]
results.df<-data.frame()
for (i in 1:length(results)){
  file.test<-read.csv(results[i],row.names=1)
  temp_dataframe<-as.data.frame(t(file.test))
  rownames(temp_dataframe)<-results[i]
  results.df<-rbind(results.df,temp_dataframe)
}
##Fix dataframe to have categories for taxa and models
results.df$Method<-rep("Ensemble",length(results.df$RMSE))
results.df$taxa<-rep(NA,length(results.df$RMSE))
results.df$taxa[1:4]<-"Bertolonia"
results.df$taxa[5:8]<-"Bignonias"
results.df$taxa[9:12]<-"Bromeliads"##the rest I added by hand
results.df$taxa[13:16]<-"Butterflies"
results.df$taxa[17:20]<-"Cambe"
results.df$taxa[21:24]<-"Hypsiboas"
results.df$taxa[25:28]<-"Melastomes"
results.df$taxa[29:32]<-"Proceratophrys"
results.df$taxa[33:36]<-"Birds"
results.df$variable<-rep(c("PD/SR_residuals","PD","PE","SR"),9)
write.csv(results.df,"All_results_Ensemble.csv")
#### plot the Rsq results per group and model 
library(ggplot2)
grafica<-ggplot(data = results.df,aes(y=Rsquared,x=variable))+  geom_bar(stat="identity", position = "dodge")
grafica+aes(fill=taxa)+ coord_flip()+theme_bw()+  xlab("Predicted variable")





#####this is used if getting the layers from folders instead of drive from GEE
# setwd("~/Dropbox/**Tesis_PHD/Near_real_time_model/Layers_yearly/")
#   ###If changing years then change numers 1:14 in loop
# crs(test_ras)<-crs(raster("MODIS_mean/mean_2002_1.tif"))
#  library(raster)    
# for (i in 1:12){
#   FileList<-list.files("MODIS_mean/",pattern=paste("_",i,".tif$",sep=""),full.names=T)
#   assign(paste("mean_train",i,sep="_"),resample(mean(stack(FileList[1:14])),test_ras,na.rm=T))
#  assign(paste("mean_test",i,sep="_"),resample(stack(FileList[15:length(FileList)]),test_ras))
# }
# for (i in 1:12){
#   FileList<-list.files("MODIS_max/",pattern=paste("_",i,".tif$",sep=""),full.names=T)
#   assign(paste("max_train",i,sep="_"),mean(stack(FileList[1:14])))
#   assign(paste("max_test",i,sep="_"),stack(FileList[15:length(FileList)]))
# }
# for (i in 1:12){
#   FileList<-list.files("MODIS_min/",pattern=paste("_",i,".tif$",sep=""),full.names=T)
#   assign(paste("min_train",i,sep="_"),mean(stack(FileList[1:14])))
#   assign(paste("min_test",i,sep="_"),stack(FileList[15:length(FileList)]))
# }
